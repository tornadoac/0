...
public class HarpyNestOffspringDialogue {
					...
					if(offspring().getPersonality().get(PersonalityTrait.EXTROVERSION) == PersonalityWeight.HIGH) {
						UtilText.nodeContentSB.append(
								"<p>"
									...
									+ " [npc.speech(Hey babe! You lookin' for a good time? "+Util.capitalizeSentence(Util.intToString(price))+" flames for an hour's ride with [npc.name]!)]"
								+ "</p>");
					} else {
						UtilText.nodeContentSB.append(
								"<p>"
				...
				if(offspring().getHistory()==Occupation.NPC_PROSTITUTE) {
					UtilText.nodeContentSB.append(
							"<p>"
								...
								+ " Sure enough, as you look across at the nest to your right, you see the familiar figure of your [npc.race] [npc.daughter] applying yet more makeup in an attempt to attract future customers."
								+ " Deciding to take a quick detour to check in on [npc.herHim], you turn to enter the nest and start to walk towards [npc.herHim]."
							+ "</p>"
							+"<p>");
				} else {
					UtilText.nodeContentSB.append(
							"<p>"
								+ "Knowing that [npc.name] lives in a nearby nest, you keep an eye out for your [npc.daughter] as you travel along the walkway."
								+ " Sure enough, as you look across at the nest to your right, you see the familiar figure of your [npc.race] [npc.daughter] lounging about beneath a large parasol."
								+ " Deciding to take a quick detour to check in on [npc.herHim], you turn to enter the nest and start to walk towards [npc.herHim]."
							+ "</p>"
							+"<p>");
				}
					...
					case NEGATIVE_FIVE_LOATHE: case NEGATIVE_FOUR_HATE:
						if(offspring().getPersonality().get(PersonalityTrait.AGREEABLENESS) == PersonalityWeight.LOW) {
							UtilText.nodeContentSB.append("Hearing your approach, [npc.name] turns towards you, and in a furious tone, [npc.she] shouts at you,"
															+" [npc.speech(What the fuck do you want?! If you're not here to apologize, then turn around and fuck off!)]");
						} else {
							UtilText.nodeContentSB.append("Hearing your approach, [npc.name] turns towards you, and with an angry scowl, [npc.she] cries out,"
															+" [npc.speech(What the hell do you want?! Unless you're here to apologize, I'm not interested in anything you have to say!)]");
						}
						break;
					case NEGATIVE_THREE_STRONG_DISLIKE: case NEGATIVE_TWO_DISLIKE:
						if(offspring().getPersonality().get(PersonalityTrait.EXTROVERSION) == PersonalityWeight.HIGH || offspring().getPersonality().get(PersonalityTrait.NEUROTICISM) == PersonalityWeight.HIGH) {
							UtilText.nodeContentSB.append("Hearing your approach, [npc.name] turns towards you, and in an angry tone, [npc.she] scowls at you,"
															+" [npc.speech(I'm not talking to you! If you're not here to apologize, you may as well turn right around and get out of here!)]");
						} else {
							UtilText.nodeContentSB.append("Hearing your approach, [npc.name] turns towards you, and with an angry scowl, [npc.she] calls out,"
															+" [npc.speech(I'm not talking to you! Unless you're here to apologize, just go away and leave me alone!)]");
						}
						break;
						...
						case NEGATIVE_FIVE_LOATHE: case NEGATIVE_FOUR_HATE: case NEGATIVE_THREE_STRONG_DISLIKE: case NEGATIVE_TWO_DISLIKE:
							if(playerPregnant) {
								if(reactedToPlayerPregnancy) {
									...
									UtilText.nodeContentSB.append("As you continue to approach your [npc.daughter], [npc.she] lets out an exasperated gasp as [npc.her] [npc.eyes] move down to rest on your pregnant belly."
																	+ " [npc.speech(You're pregnant?! Again?! Do this next lot a favor and don't try to ruin their lives, like you have with mine!)]");
								}
							...
							if(offspringPregnant) {
								if(reactedToOffspringPregnancy) {
									UtilText.nodeContentSB.append(
											"Stroking [npc.her] swollen stomach, [npc.she] calls out to you as you draw near,"
					...
					switch(offspring().getAffectionLevel(Main.game.getPlayer())) {
						case NEGATIVE_FIVE_LOATHE: case NEGATIVE_FOUR_HATE: case NEGATIVE_THREE_STRONG_DISLIKE: case NEGATIVE_TWO_DISLIKE:
							UtilText.nodeContentSB.append(
									...
									+ " Realizing that you're not going to be able to have a proper conversation with [npc.herHim] until you've sincerely apologized, you're only left with a few options...");
							break;
						case NEGATIVE_ONE_ANNOYED:
							UtilText.nodeContentSB.append(
									...
									+ " Realizing that you're not going to be able to have a proper conversation with [npc.herHim] until you've apologized for your actions, you're only left with a few options...");
							break;
						case ZERO_NEUTRAL: case POSITIVE_ONE_FRIENDLY: case POSITIVE_TWO_LIKE: case POSITIVE_THREE_CARING: case POSITIVE_FOUR_LOVE: case POSITIVE_FIVE_WORSHIP:
							UtilText.nodeContentSB.append(
					...
					switch(offspring().getAffectionLevel(Main.game.getPlayer())) {
						...
						case NEGATIVE_THREE_STRONG_DISLIKE: case NEGATIVE_TWO_DISLIKE:
							UtilText.nodeContentSB.append(
									...
									+ " It really seems as though [npc.she] dislikes you, and if you wanted to have a proper conversation with [npc.herHim], it looks like you'll have to apologize for the way you've acted towards [npc.herHim]...");
							break;
						case NEGATIVE_ONE_ANNOYED:
							UtilText.nodeContentSB.append(
					...
					return new Response("Hug", "Hug [npc.name].", OFFSPRING_ENCOUNTER_TALKING) {
						...
						public void effects() {
							if(!offspring().hasFlag(NPCFlagValue.flagOffspringIntroduced)) {
								...
								Main.game.getTextStartStringBuilder().append(
										"<p>"
											+ "Deciding that [npc.namePos] in need of a hug, you step forwards and wrap your [pc.arms] around your [npc.daughter], before pulling [npc.herHim] into your loving embrace."
					...
					return new Response("Kiss", "Give [npc.name] a hug and a kiss.", OFFSPRING_ENCOUNTER_TALKING) {
						...
						public void effects() {
							if(!offspring().hasFlag(NPCFlagValue.flagOffspringIntroduced)) {
								...
								Main.game.getTextStartStringBuilder().append(
										"<p>"
											+ "Deciding that [npc.namePos] in need of some parental love, you step forwards and wrap your [pc.arms] around your [npc.daughter],"
												+ " before planting a wet kiss on [npc.her] cheek and pulling [npc.herHim] into a loving hug."
											+ " [npc.She] returns your gesture, hugging you close to [npc.her] warm body as you both share a happy moment, before breaking away from each other."
											+ " Looking into [npc.her] [npc.eyes+], you greet your [npc.daughter] once again,"
											+ " [pc.speech(Hello, [npc.Name], how have you been?)]"
						...
						public void effects() {
							if(!offspring().hasFlag(NPCFlagValue.flagOffspringIntroduced)) {
								if(offspring().isAttractedTo(Main.game.getPlayer())) {
									Main.game.getTextStartStringBuilder().append(
											"<p>"
												+ "You're the first to recover from the shock of your surprise meeting, and, with one thought in your mind, you step forwards, before wrapping your [pc.arms] around [npc.namePos] back."
											...
											+ "<p>"
												...
												+ " Darting your [pc.hands] up to take hold of [npc.her] head, you passionately thrust your [pc.tongue+] into [npc.her] mouth,"
													+ " desperately [pc.moaning] as you pull your surprised [npc.daughter] into your embrace and start to make out with [npc.herHim]."
											+ "</p>"
											+ "<p>"
											...
											+ "</p>"
											+ "<p>"
												+ "You continue with this for some time, and, moving your [pc.hands] down over [npc.her] body, you eagerly grope, stroke, and squeeze every inch of [npc.her] body that's within your reach."
											...
											+ "</p>"
											+ "<p>"
												+ "You can't help but laugh at [npc.namePos] stunned reaction,"
									...
									Main.game.getTextStartStringBuilder().append(
											...
											+ "</p>"
											+ "<p>"
												+ "You can't help but continue to smile at [npc.namePos] reaction."
								...
								if(offspring().isAttractedTo(Main.game.getPlayer())) {
									Main.game.getTextStartStringBuilder().append(
											...
											+ "</p>"
											+ "<p>"
												+ "You can't help but smile at [npc.namePos] happy reaction."
							...
							if(!offspring().hasFlag(NPCFlagValue.flagOffspringIntroduced)) {
								...
								Main.game.getTextStartStringBuilder().append(
										"<p>"
											...
											+ " [pc.speech(I don't approve of what you're doing!)]"
										+ "</p>"
										+ "<p>"
											+ "Obviously deciding that it's better to just endure your lecture rather than trying to talk back, [npc.name] just rolls [npc.her] [npc.eyes] and crosses [npc.her] [npc.arms]."
										...
										+ "</p>"
										+ "<p>"
											+ "Eventually, however, you run out of steam, and [npc.name] lets out an annoyed sigh, before turning to you."
											...
											Main.game.getTextStartStringBuilder().append(
												"<p>"
													...
													+ (offspring().getPersonality().get(PersonalityTrait.EXTROVERSION) == PersonalityWeight.HIGH || offspring().getPersonality().get(PersonalityTrait.NEUROTICISM) == PersonalityWeight.HIGH
															?" [npc.speech(Fine, fuck off then! See if I care! Don't bother coming back unless you're gonna apologize!)]"
															:" [npc.speech(Eugh, fine! See if I care! Oh, and don't bother coming back unless you're gonna apologize!)]")
												+"</p>");
											break;
										case NEGATIVE_ONE_ANNOYED:
											Main.game.getTextStartStringBuilder().append(
													"<p>"
														...
														+ (offspring().getPersonality().get(PersonalityTrait.EXTROVERSION) == PersonalityWeight.HIGH || offspring().getPersonality().get(PersonalityTrait.NEUROTICISM) == PersonalityWeight.HIGH
																?" [npc.speech(Fine, see you later! And you'd better be ready to apologize by then!)]"
	...
	public static final DialogueNode OFFSPRING_ENCOUNTER_APOLOGY = new DialogueNode("A familiar face", "", true) {
		...
		public String getContent() {
			...
			if(offspring().hasFlag(NPCFlagValue.flagOffspringFightApologyNeeded) && offspring().hasFlag(NPCFlagValue.flagOffspringRapeApologyNeeded)) {
				...
				UtilText.nodeContentSB.append(
						"<p>"
							+ "Deciding that you need to apologize for treating your [npc.daughter] so poorly, you step forwards and start to speak,"
			...
			if(offspring().getPersonality().get(PersonalityTrait.EXTROVERSION) == PersonalityWeight.HIGH || offspring().getPersonality().get(PersonalityTrait.NEUROTICISM) == PersonalityWeight.HIGH) {
				UtilText.nodeContentSB.append("<p>"
						...
						+ "</p>"
						+ "<p>"
							+ "You continue to apologize to your [npc.daughter] for some time, and while [npc.she] frequently interrupts you to berate you, you can tell that your words are having a positive effect."
						...
						+ "</p>"
						+ "<p>"
							+ "With that, [npc.she] turns [npc.her] back on you and refuses to continue the conversation."
						+ "</p>"
						+ "<p>"
				...
				UtilText.nodeContentSB.append("<p>"
					...
					+ "</p>"
					+ "<p>"
						+ "You continue to apologize to your [npc.daughter] for some time, and while [npc.she] frequently turns [npc.her] back on you, before spinning around to mention yet another thing you've done wrong,"
	...
	private static String getFooterInformationText() {
		...
		if(offspring().isAttractedTo(Main.game.getPlayer())) {
			footerSB.append(
					"<p>"
						+ "<i>You notice [npc.namePos] gaze flick down as [npc.she] tries to take an unnoticed peek at your body."
		...
		public String getContent() {
			...
			if(!offspring().hasFlag(NPCFlagValue.flagOffspringApartmentIntroduced)) {
				UtilText.nodeContentSB.append("<p>"
							...
							+ " Following in [npc.her] footsteps, you allow your [npc.daughter] to take the lead as [npc.she] guides you through the crowds of harpies to a stair access building on one side of the nest."
							+ " Pushing open the door, your [npc.daughter] leads you inside, before heading down the staircase to the floor immediately below the nest."
						+ "</p>"
						+ "<p>"
							+ "Leading you down a corridor, [npc.name] produces a set of keys, before unlocking one of the doors, stepping inside, and calling for you to follow [npc.herHim] in."
							+ " As you step into your [npc.daughter]'s home, you find that the interior is far nicer, and larger, than you'd expected."
						+ "</p>"
						+ "<p>"
							+ "Looking down the length of the apartment, you see that there are both windows and a glass door set into the far end, allowing access to what appears to be a small balcony."
						...
						public void effects() {
							Main.game.getTextStartStringBuilder().append(
									...
									+ "<p>"
										...
										+ " You both say your farewells, and after just a few moments, you find yourself back out in the nests once again..."
									+ "</p>");
							...
							public void effects() {
								Main.game.getTextStartStringBuilder().append(
										...
										+ "<p>"
											...
											+ " You both say your farewells, and after just a few moments, you find yourself back out in the walkways once again..."
										+ "</p>");
								offspring().setFlag(NPCFlagValue.flagOffspringApartmentIntroduced, true);
							}
							@Override
			...
			if(offspring().getHistory()==Occupation.NPC_PROSTITUTE) {
				...
				if(offspring().getPersonality().get(PersonalityTrait.EXTROVERSION) == PersonalityWeight.HIGH) {
					UtilText.nodeContentSB.append(
							"<p>"
								+ "[npc.She] leans back on the sofa and smiles."
								+ " [npc.speech(Well, you know, there's always <i>someone</i> dumb enough to try messing with our nest, so I never have any problems with money."
									+ " I mean, if they didn't want to get mugged, they wouldn't come trying to bother our matriarch!)]"
							+ "</p>"
							+ "<p>"
					...
					UtilText.nodeContentSB.append(
							"<p>"
								+ "[npc.She] leans back on the sofa and smiles."
								+ " [npc.speech(Well, you know, there's always <i>someone</i> who's coming along trying to mess with our nest, so I never have any problems with money."
	...
	public static final DialogueNode OFFSPRING_ENCOUNTER_SEX = new DialogueNode("", "", true) {
		...
		public String getContent() {
			...
			if(offspring().isAttractedTo(Main.game.getPlayer())) {
				UtilText.nodeContentSB.append(
						"<p>"
							+ "[npc.NamePos] mouth opens a little as [npc.she] lets out a desperate little whine."
			...
			if (index == 1) {
					...
					public void effects() {
						Main.game.getTextStartStringBuilder().append(
								"<p>"
									...
									+ " Leaning in over [npc.her] shoulder as you press yourself to [npc.herHim], you apologize for what you've done,"
								...
								+ "<p>"
									+ "[npc.Name] walks you to the door in silence, and after trying to say sorry once more, only to have your [npc.daughter] slam the door in your face,"
										+ " you find yourself back out on the walkways once again..."
								+ "</p>");
						Main.game.getTextEndStringBuilder().append(offspring().incrementAffection(Main.game.getPlayer(), 25));
						offspring().setFlag(NPCFlagValue.flagOffspringFightApologyNeeded, false);
					}
				...
				if(offspring().isAttractedTo(Main.game.getPlayer()) || !Main.game.isNonConEnabled()) {
					return new ResponseSex("Submit",
							...
							+ "</p>"
							+ "<p>"
								+ "A hungry grin spreads across [npc.namePos] face, before [npc.she] pulls you to your feet and starts to kiss you once more..."
							+ "</p>") {
						@Override
						public void effects() {
					...
					public void effects() {
						Main.game.getTextStartStringBuilder().append(
								...
								+ "<p>"
									...
									+ " Turning the handle, you open the door and make your exit, and within a few minutes, find yourself traveling down the walkways once again..."
								+ "</p>");
					}
				};
		...
		public String getContent() {
			if(offspring().isAttractedTo(Main.game.getPlayer())) {
				...
				return UtilText.parse(offspring(),
						...
						+ "</p>"
						+ "<p>"
							+ "Not being in any kind of situation to refuse, you apologize to your [npc.daughter] for attacking [npc.herHim]."
						...
						+ "<p>"
							...
							+ " Leaving you in the middle of a nearby walkway, [npc.she] calls out behind [npc.herHim] as [npc.she] takes [npc.her] leave,"
	...
	public static final DialogueNode AFTER_SEX_CONSENSUAL = new DialogueNode("Post-sex", "", true) {
		...
		public String getContent() {
			if(Sex.getNumberOfOrgasms(Sex.getActivePartner()) >= 1) {
				return UtilText.parse(offspring(),
						"<p>"
							+ "Disentangling yourselves from each other's clutches, you both sink down onto [npc.namePos] sofa once again."
			...
			if(offspring().hasFlag(NPCFlagValue.fightOffspringInApartment)) {
				if(!offspring().isAttractedTo(Main.game.getPlayer()) && Main.game.isNonConEnabled()) {
					...
					if(Sex.getNumberOfOrgasms(Sex.getActivePartner()) >= 1) {
						...
						return UtilText.parse(offspring(),
								"<p>"
									+ "As you step back from [npc.name], [npc.she] grabs on to the edge of the sofa and sinks to the floor, letting out a desperate whine as [npc.she] realizes that you've finished."
						...
						if(offspring().hasFlag(NPCFlagValue.fightOffspringInApartment)) {
							...
							if(!offspring().isAttractedTo(Main.game.getPlayer()) && Main.game.isNonConEnabled()) {
								...
								if(Sex.getNumberOfOrgasms(Sex.getActivePartner()) >= 1) {
									...
									Main.game.getTextStartStringBuilder().append("Leaving [npc.herHim] to get some pleasure by [npc.herself], you set off and continue on your journey through the nests.");
								}
							}
						}
						Main.game.getTextStartStringBuilder().append("</p>");
		...
		public String getContent() {
			if(offspring().hasFlag(NPCFlagValue.fightOffspringInApartment)) {
				return UtilText.parse(
						...
						+ "</p>"
						+ "<p>"
							+ "With that, [npc.she] turns around and walks off, leaving you to get your things in order before setting off into the nests once again..."
						+ "</p>");
			} else {
				return UtilText.parse(
						"<p>"
		...
		public String getContent() {
			...
			if(enslavementClothing.getClothingType().equals(ClothingType.NECK_SLAVE_COLLAR)) {
				return UtilText.parse(offspring(),
						...
						+ "</p>"
						+ "<p>"
							+ "As the collar's arcane enchantment recognizes its new wearer as being a criminal, ominous purple lettering starts to glow around the metal band, reading;"
								+ " <i style='color:"+Color.GENERIC_ARCANE.toWebHexString()+";'>New Slave Accepted. Identification: [style.boldArcane("+offspring().getNameIgnoresPlayerKnowledge()+")], [npc.race].</i>"
							+ " Finally realizing what's going on, [npc.name] looks up at you with fear in [npc.her] [npc.eyes]."
				...
				return UtilText.parse(offspring(),
						...
						+ "</p>"
						+ "<p>"
							+ "As the arcane enchantment recognizes its new wearer as being a criminal, ominous purple lettering is projected into the air, reading;<br/>"
