...
public class LilayasRoom {
	...
	public static final DialogueNode CAUGHT = new DialogueNode("Lilaya's Room", ".", true) {
		...
		public Response getResponse(int responseTab, int index) {
			if(index==1) {
				return new Response("Apologize", "Say sorry to Rose.", APOLOGY) {
					@Override
					public void effects() {
			...
			if(index==1) {
				return new ResponseSex("Submit", "Let Rose push you back on to the bed and fuck you.",
					...
					public void effects() {
						...
						Main.game.getNpc(Rose.class).equipClothingFromNowhere(AbstractClothingType.generateClothing(ClothingType.BDSM_PENIS_STRAPON, Color.CLOTHING_PURPLE_DARK, false), true, Main.game.getNpc(Rose.class));
					}
				};
