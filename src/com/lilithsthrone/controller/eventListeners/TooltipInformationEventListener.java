	...
	public void handleEvent(Event event) {
		...
		if (statusEffect != null) {
			...
			tooltipSB.append("<body>"
					+ "<div class='title'>" + Util.capitalizeSentence(statusEffect.getName(owner)) + "</div>");
			...
			tooltipSB.append("<div class='subTitle-picture' style='white-space: nowrap'>");
				if (!statusEffect.getModifiersAsStringList(owner).isEmpty()) {
					...
					tooltipSB.append("<span style='color:" + Color.TEXT_GREY.toWebHexString() + ";'>No bonuses</span>");
				}
			...
			if(owner.hasStatusEffect(statusEffect)) {
				if (owner.getStatusEffectDuration(statusEffect) != -1) {
					if (statusEffect.isCombatEffect()) {
						...
						Color timerColor = Color.STATUS_EFFECT_TIME_HIGH;

						if(timerHeight>100) {
							timerHeight=100;
							timerColor = Color.STATUS_EFFECT_TIME_OVERFLOW;
						} else if(timerHeight<15) {
							timerColor = Color.STATUS_EFFECT_TIME_LOW;
						} else if (timerHeight<50) {
							timerColor = Color.STATUS_EFFECT_TIME_MEDIUM;
						}

						tooltipSB.append("<div class='subTitle'><b>Time remaining: "
								+ "<b style='color:"+timerColor.toWebHexString()+";'>" + (owner.getStatusEffectDuration(statusEffect) / 60) + ":" + String.format("%02d", (owner.getStatusEffectDuration(statusEffect) % 60)) + "</b>"
								+ "</div>");
...
//			Main.mainController.setTooltipSize(360, height+8);
			...
			// Title:
			tooltipSB.setLength(0);
			tooltipSB.append("<div class='title'>" + Util.capitalizeSentence(perk.getName(owner)) + "</div>");

			if(perk.isEquippableTrait()) {
				tooltipSB.append("<div class='subTitle' style='color:"+Color.TRAIT.toWebHexString()+";'>Trait</div>");
			} else {
				tooltipSB.append("<div class='subTitle' style='color:"+Color.PERK.toWebHexString()+";'>Perk</div>");
			}
			...
			if (!perk.getModifiersAsStringList().isEmpty()) {
				...
				tooltipSB.append("<b style='color:" + Color.PERK.toWebHexString() + ";'>Perk</b>" + "<br/><span style='color:" + Color.TEXT_GREY.toWebHexString() + ";'>None</span>");
			tooltipSB.append("</div>");
			...
			// Title:
			tooltipSB.setLength(0);
			tooltipSB.append("<div class='title'>" + Util.capitalizeSentence(levelUpPerk.getName(owner)) + "</div>");
			...
			if(levelUpPerk.isEquippableTrait()) {
				if(levelUpPerk.getPerkCategory()==PerkCategory.JOB) {
					tooltipSB.append("<div class='subTitle' style='color:"+Color.TRAIT.toWebHexString()+";'>Job-related trait cannot be removed.</div>");

				} else {
					if(!owner.hasPerkInTree(perkRow, levelUpPerk)) {
						if(!PerkManager.MANAGER.isPerkAvailable(owner, perkRow, levelUpPerk)) {
							tooltipSB.append("<div class='subTitle' style='color:"+Color.GENERIC_BAD.toWebHexString()+";'>Purchasing requires a connecting perk or trait.</div>");
						} else {
							tooltipSB.append("<div class='subTitle' style='color:"+Color.GENERIC_MINOR_GOOD.toWebHexString()+";'>Click to purchase trait.</div>");
						}
					} else {
						if(owner.getTraits().contains(levelUpPerk)) {
							tooltipSB.append("<div class='subTitle' style='color:"+Color.GENERIC_MINOR_BAD.toWebHexString()+";'>Click to unequip trait.</div>");
						} else {
							if(owner.getTraits().size()==GameCharacter.MAX_TRAITS) {
								tooltipSB.append("<div class='subTitle' style='color:"+Color.GENERIC_BAD.toWebHexString()+";'>Maximum traits activated.</div>");
							} else {
								tooltipSB.append("<div class='subTitle' style='color:"+Color.TRAIT.toWebHexString()+";'>Click to equip trait.</div>");
							}
						}
					}
				}
				...
				if(!owner.hasPerkInTree(perkRow, levelUpPerk) && !levelUpPerk.isHiddenPerk()) {
					if(!PerkManager.MANAGER.isPerkAvailable(owner, perkRow, levelUpPerk)) {
						...
						tooltipSB.append("<div class='subTitle' style='color:"+Color.GENERIC_MINOR_GOOD.toWebHexString()+";'>Click to purchase perk.</div>");
					}
				} else {
					tooltipSB.append("<div class='subTitle' style='color:"+Color.PERK.toWebHexString()+";'>You already own this perk!</div>");
				}
			}
			...
			// Title:
			tooltipSB.setLength(0);
			tooltipSB.append("<div class='title'>Set Desire: <b style='color:" + desire.getColor().toWebHexString() + ";'>"+Util.capitalizeSentence(desire.getName())+"</b></div>");
			...
			if(owner.hasFetish(fetish) && desire!=FetishDesire.FOUR_LOVE) {
				tooltipSB.append("<div class='description' style='height:53px'>Your desire is [style.boldBad(locked)] to <b style='color:"+FetishDesire.FOUR_LOVE.getColor().toWebHexString()+";'>"+FetishDesire.FOUR_LOVE.getName()+"</b>,"
			...
			if(fetishExperience) {
				...
				tooltipSB.setLength(0);
				tooltipSB.append("<div class='title'>" + Util.capitalizeSentence(fetish.getName(owner)) + " fetish</div>");
				FetishLevel level = FetishLevel.getFetishLevelFromValue(owner.getFetishExperience(fetish));
				tooltipSB.append("<div class='subTitle'>Level "+level.getNumeral()+": <span style='color:"+level.getColor().toWebHexString()+";'>"+Util.capitalizeSentence(level.getName())+"</span>"
									+ " <span style='color:" + Color.TEXT_GREY.toWebHexString() + ";'>|</span> " + owner.getFetishExperience(fetish) +" / "+ level.getMaximumExperience() + " xp" + "</div>");
				...
				if(!fetish.getFetishesForAutomaticUnlock().isEmpty()) {
					...
					for (Fetish f : fetish.getFetishesForAutomaticUnlock())
						tooltipSB.append("<br/>[style.boldArcane(" + Util.capitalizeSentence(f.getName(Main.game.getPlayer()))+")]");
					tooltipSB.append("</div>");
				}
				...
				if (!fetish.getModifiersAsStringList(owner).isEmpty()) {
					...
					tooltipSB.append("<b style='color:" + Color.FETISH.toWebHexString() + ";'>Fetish</b>" + "<br/><span style='color:" + Color.TEXT_GREY.toWebHexString() + ";'>None</span>");
				}
				tooltipSB.append("</div>");
			...
			// Title:
			tooltipSB.setLength(0);
			tooltipSB.append("<div class='title'>" + Util.capitalizeSentence(specialAttack.getName()) + "</div>");

			// Attribute modifiers:
			tooltipSB.append("<div class='subTitle-picture'>"
					+ "<b style='color:" + Color.SPECIAL_ATTACK.toWebHexString() + ";'>Special Attack</b><br/>"
					+ "<b>"
					...
					+ "</b>"
					+ " <b style='color:" + specialAttack.getDamageType().getMultiplierAttribute().getColor().toWebHexString() + ";'>" + specialAttack.getDamageType().getName()
					+ "</b> damage");

			tooltipSB.append("<br/><b style='color:" + Color.SPECIAL_ATTACK.toWebHexString() + ";'>Applies</b>");
			if (!specialAttack.getStatusEffects().isEmpty()) {
				for (Entry<StatusEffect, Integer> e : specialAttack.getStatusEffects().entrySet())
					tooltipSB.append("<br/><b style='color:" + e.getKey().getColor().toWebHexString() + ";'>" + Util.capitalizeSentence(e.getKey().getName(owner)) + "</b> for " + e.getValue() + " turn" + (e.getValue() > 1 ? "s" : ""));
			} else
				tooltipSB.append("<br/><span style='color:" + Color.TEXT_GREY.toWebHexString() + ";'>No effects</span>");
			tooltipSB.append("</div>");
			...
			// Title:
			tooltipSB.setLength(0);
			tooltipSB.append("<div class='title'>" + Util.capitalizeSentence(spell.getName()) + "</div>");
			...
			if(spell.getDamage(Main.game.getPlayer())>0) {
				tooltipSB.append(
						"<b>Base "+spell.getDamage(owner)+"</b> <b style='color:"+ spell.getDamageType().getMultiplierAttribute().getColor().toWebHexString() + ";'>" + Util.capitalizeSentence(spell.getDamageType().getName()) + " Damage</b><br/>"
						+"<b>"
						...
						+ "</b>"
						+ " <b style='color:"+ spell.getDamageType().getMultiplierAttribute().getColor().toWebHexString() + ";'>" + Util.capitalizeSentence(spell.getDamageType().getName()) + " Damage</b><br/>");
			}
			...
			if(!spell.getModifiersAsStringList().isEmpty()) {
				...
				tooltipSB.append("<span style='color:" + Color.TEXT_GREY.toWebHexString() + ";'>No effects</span><br/>");
			}
			tooltipSB.append("</div>");
			...
			tooltipSB.append(
					...
					+ "</div>"
					+ "<div class='subTitle'>"
						+ "<b style='color:" + Color.GENERIC_BAD.toWebHexString() + ";'>Costs</b> <b>" + (spell.getModifiedCost(owner)) + "</b> <b style='color:" + Color.ATTRIBUTE_MANA.toWebHexString() + ";'>aura</b>"
					+ "</div>");
			...
			// Title:
			tooltipSB.setLength(0);
			tooltipSB.append("<div class='title'>" + Util.capitalizeSentence(spellUpgrade.getName()) + "</div>");
			...
			tooltipSB.append(
					...
					+ "<div class='subTitle'>"
						+ (owner.hasSpellUpgrade(spellUpgrade)
								?"[style.boldExcellent(Owned)] (Cost <b style='color:"+spellUpgrade.getSpellSchool().getColor().toWebHexString()+";'>"+spellUpgrade.getPointCost()+"</b> Point"+(spellUpgrade.getPointCost()==1?"":"s")+")"
								:(owner.getSpellUpgradePoints(spellUpgrade.getSpellSchool()) >= spellUpgrade.getPointCost()
										?"Costs <b style='color:"+spellUpgrade.getSpellSchool().getColor().toWebHexString()+";'>"+spellUpgrade.getPointCost()+"</b> Point"+(spellUpgrade.getPointCost()==1?"":"s")+" - [style.colorGood(Can afford!)]"
										:"Costs <b style='color:"+spellUpgrade.getSpellSchool().getColor().toWebHexString()+";'>"+spellUpgrade.getPointCost()+"</b> Point"+(spellUpgrade.getPointCost()==1?"":"s")+" - [style.colorBad(Cannot afford!)]"))
					+ "</div>");
			...
			if (attribute == Attribute.MAJOR_PHYSIQUE
				...
				tooltipSB.setLength(0);
				tooltipSB.append("<div class='title' style='color:" + attribute.getColor().toWebHexString() + ";'>" + Util.capitalizeSentence(attribute.getName()) + "</div>"

						+ "<div class='subTitle-third'>" + "<b style='color:" + Color.TEXT_GREY.toWebHexString() + ";'>Core</b><br/>"
						+ (owner.getBaseAttributeValue(attribute) > 0 ? "<span style='color: " + Color.GENERIC_EXCELLENT.getShades()[1] + ";'>" : "<span>") + String.format("%.2f", owner.getBaseAttributeValue(attribute)) + "</span>" + "</div>"

						+ "<div class='subTitle-third'>" + "<b style='color:" + Color.TEXT_GREY.toWebHexString() + ";'>Bonus</b><br/>"
						+ ((owner.getBonusAttributeValue(attribute)) > 0 ? "<span style='color: " + Color.GENERIC_GOOD.getShades()[1] + ";'>"
								: ((owner.getBonusAttributeValue(attribute)) == 0 ? "<span style='color:" + Color.TEXT_GREY.toWebHexString() + ";'>" : "<span style='color: " + Color.GENERIC_BAD.getShades()[1] + ";'>"))
						+ String.format("%.2f", owner.getBonusAttributeValue(attribute))+ "</span>" + "</div>"

						+ "<div class='subTitle-third'>" + "<b style='color:" + attribute.getColor().toWebHexString() + ";'>Total</b><br/>" + String.format("%.2f", owner.getAttributeValue(attribute))
						+ "</span>" + "</div>");
				...
				tooltipSB.append("<div class='title'>"
												+ "<span style='color:"+currentAttributeStatusEffect.getColor().toWebHexString()+";'>"
				...
				if(owner.isRaceConcealed()) {
					...
					tooltipSB.setLength(0);
					tooltipSB.append("<div class='title' style='color:" + Color.RACE_UNKNOWN.toWebHexString() + ";'>"
							+ "Unknown Race!"
							+ "</div>");
					...
					tooltipSB.setLength(0);
					tooltipSB.append("<div class='title' style='color:" + owner.getRace().getColor().toWebHexString() + ";'>"
							+(owner.getRaceStage().getName()!=""?"<b style='color:"+owner.getRaceStage().getColor().toWebHexString()+";'>" + Util.capitalizeSentence(owner.getRaceStage().getName())+"</b> ":"")
							+ "<b style='color:"+owner.getSubspecies().getColor(owner).toWebHexString()+";'>"
							+ (owner.isFeminine()?Util.capitalizeSentence(owner.getSubspecies().getSingularFemaleName(owner)):Util.capitalizeSentence(owner.getSubspecies().getSingularMaleName(owner)))
							+ "</b>"
							+ "</div>");
					...
					if (owner.getHornType() != HornType.NONE) {
						tooltipSB.append(getBodyPartDiv((owner.hasHorns()?Util.capitalizeSentence(owner.getHornName()):"Horns"), owner.getHornRace(), owner.getHornCovering(), owner.isHornBestial()));
					} else {
					...
					if(!owner.isPlayer() && !owner.isAreaKnownByCharacter(CoverableArea.PENIS, Main.game.getPlayer())) {
						...
						if (owner.hasPenis()) {
							tooltipSB.append(getBodyPartDiv("Penis", owner.getPenisRace(), owner.getPenisCovering(), owner.isPenisBestial()));
						} else {
						...
						tooltipSB.append("</div>"
								+ "<div style='float: left;'>"
									+ "<img id='CHARACTER_IMAGE' style='"+(revealed?"":"-webkit-filter: brightness(0%);")
										...
										+(revealed?"":"<p style='position:absolute; top:33%; right:0; width:"+imageWidth+"; font-weight:bold; text-align:center; color:"+Color.BASE_GREY.toWebHexString()+";'>Unlocked through sex!</p>")
								+ "</div>");
					}
				}
				...
				Main.mainController.setTooltipContent(UtilText.parse(
						"<div class='title' style='color:" + attribute.getColor().toWebHexString() + ";'>" + Util.capitalizeSentence(attribute.getName()) + "</div>"

						+ "<div class='subTitle-third'>"
						+ "<b style='color:" + Color.TEXT_GREY.toWebHexString() + ";'>Core</b><br/>"
						+ (owner.getBaseAttributeValue(attribute) > 0 ? "<span style='color: " + Color.GENERIC_EXCELLENT.getShades()[1] + ";'>" : "<span>") + String.format("%.2f", owner.getBaseAttributeValue(attribute))
						+ "</span>"
						+ "</div>"
						+ "<div class='subTitle-third'>"
						+ "<b style='color:"
						+ Color.TEXT_GREY.toWebHexString()
						+ ";'>Bonus</b><br/>"
						+ ((owner.getBonusAttributeValue(attribute)) > 0 ? "<span style='color: "
								+ Color.GENERIC_GOOD.getShades()[1]
								+ ";'>"
								: ((owner.getBonusAttributeValue(attribute)) == 0 ? "<span style='color:"
										+ Color.TEXT_GREY.toWebHexString()
										+ ";'>"
										: "<span style='color: "
												+ Color.GENERIC_BAD.getShades()[1]
												+ ";'>"))
						...
						+ "<div class='subTitle-third'>"
						+ "<b style='color:"
						+ attribute.getColor().toWebHexString() + ";'>Total</b><br/>" + String.format("%.2f", owner.getAttributeValue(attribute)) + "</span>"
						+ "</div>"
...
//			} else {
				...
				tooltipSB.append(UtilText.parse(owner,
						"<div class='title' style='color:" + Femininity.valueOf(owner.getFemininityValue()).getColor().toWebHexString() + ";'>"
						...
						+"<div class='subTitle' style='margin-bottom:4px;'>Level " + owner.getLevel() + " <span style='color:" + Color.TEXT_GREY.toWebHexString() + ";'>| "
...
//			}
			...
			tooltipSB.setLength(0);
			tooltipSB.append("<div class='title'>" + "<b style='color:" + Main.game.getCurrentWeather().getColor().toWebHexString() + ";'>" + Util.capitalizeSentence(Main.game.getCurrentWeather().getName()) + "</b>" + "</div>"
			...
			tooltipSB.append("<div class='title'>Protection</div>"
					+ "<div class='subTitle'>"
					+ (owner.isWearingCondom()?"<span style='color:"+Color.GENERIC_GOOD.toWebHexString()+";'>Wearing Condom</span>":"<span style='color:"+Color.GENERIC_BAD.toWebHexString()+";'>No Condom</span>")
					+"</div>");
			...
			tooltipSB.append(
					...
					+ "This scene was written by <b style='color:"+Color.ANDROGYNOUS.toWebHexString()+";'>"
			...
			Main.mainController.setTooltipContent(UtilText.parse(
					"<div class='title'>"+Util.capitalizeSentence(concealedSlot.getName())+" - [style.boldBad(Concealed!)]</div>"
					+ "<div class='description'>"
			...
			// Title:
			tooltipSB.setLength(0);
			tooltipSB.append("<div class='title'>" + Util.capitalizeSentence(loadedEnchantment.getName()) + "</div>");

			if(loadedEnchantment.isSuitableItemAvailable()) {
				tooltipSB.append("<div class='subTitle' style='color:"+Color.GENERIC_GOOD.toWebHexString()+";'>Suitable item in inventory</div>");
			} else {
				tooltipSB.append("<div class='subTitle' style='color:"+Color.GENERIC_BAD.toWebHexString()+";'>No suitable item in inventory</div>");
			}
	...
	private String getBodyPartDiv(String name, Race race, BodyCoveringType covering, boolean bestial) {
		...
		return "<div class='subTitle' style='font-weight:normal; text-align:left; margin-top:2px; white-space: nowrap;'>"
					+ name
					+ ": "+ (bestial?"[style.colorBestial(Feral )]":"")+"<span style='color:" + race.getColor().toWebHexString() + ";'>"+Util.capitalizeSentence(raceName) + "</span> - "
					+ owner.getCovering(covering).getColorDescriptor(owner, true, true) + " " + owner.getCovering(covering).getName(owner)
				+"</div>";
	}

	private String getEmptyBodyPartDiv(String name, String description) {
		return "<div class='subTitle' style='font-weight:normal; text-align:left; margin-top:2px; white-space: nowrap;'>"+ name + ": <span style='color:" + Color.TEXT_GREY.toWebHexString() + ";'>"+description+"</span></div>";
	}


	private String extraAttributeTableRow(GameCharacter owner, String type, Attribute damage, Attribute resist) {
		return "<div class='subTitle-third combatValue'>" + "<span style='color:" + damage.getColor().toWebHexString() + ";'>" + type + "</span>" + "</div>" + "<div class='subTitle-third combatValue'>"
				+ (owner.getAttributeValue(damage) > damage.getBaseValue()
										? "<span style='color:" + Color.GENERIC_GOOD.toWebHexString() + ";'>"
										: (owner.getAttributeValue(damage) < damage.getBaseValue()
												? "<span style='color:" + Color.GENERIC_BAD.toWebHexString() + ";'>"
												: ""))
				+ owner.getAttributeValue(damage)
				+ "</span>" + "</div>" + "<div class='subTitle-third combatValue'>"
				+ (resist == null ? "0.0"
						: (owner.getAttributeValue(resist) > 0 ? "<span style='color:" + Color.GENERIC_GOOD.toWebHexString() + ";'>"
								: (owner.getAttributeValue(resist) < 0 ? "<span style='color:" + Color.GENERIC_BAD.toWebHexString() + ";'>" : "")) + owner.getAttributeValue(resist) + "</span>")
				+ "</div>";
	}

	private String extraAttributeBonus(GameCharacter owner, Attribute bonus) {
		return "<div class='subTitle-half'>" + "<span style='color:"
				+ bonus.getColor().toWebHexString() + ";'>" + Util.capitalizeSentence(bonus.getName()) + "</span><br/>" + (owner.getAttributeValue(bonus) > bonus.getBaseValue()
						? "<span style='color:" + Color.GENERIC_GOOD.toWebHexString() + ";'>" : (owner.getAttributeValue(bonus) < bonus.getBaseValue() ? "<span style='color:" + Color.GENERIC_BAD.toWebHexString() + ";'>" : ""))
